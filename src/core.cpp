...
#ifdef _DEBUG
    if (! m_raw_texture) {
        throw std::logic_error("Cannot access ScreenBufferView after destroy() has been called.");
    }

    if (y < 0 || y >= m_height || x < 0 || x >= m_width) {
        throw std::logic_error("Cannot write to ScreenBufferView out of bounds.");
    }
#endif
